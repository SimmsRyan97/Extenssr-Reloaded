import { Uniform, UniformType } from 'postprocessing/frontend/uniform'
import { Dirtifier, OffscreenContext } from './webgl_context_wrapper'
import AiOverlay from './ai_overlay'
import Texture from './texture'
import Material from './material'
import { ChromeInjectedToContentBroker } from 'messaging/content_to_injected_broker'
import { Shader } from 'postprocessing/frontend/shader_pass'
...
export class PostprocessHandler {
    ...
    constructor(canvas: HTMLCanvasElement, webglContext: WebGLRenderingContext, offscreenCanvas: HTMLCanvasElement, offscreenContext: OffscreenContext, dirtifier: Dirtifier, broker: ChromeInjectedToContentBroker) {
        this.canvas = canvas
        ...
        this.aiOverlay = new AiOverlay(offscreenCanvas, webglContext, dirtifier, broker)
        this.broker = broker
        this.defaultMaterial = new Material(webglContext)
        broker.createListener('changePov', '', ([vx, vy]) => {
            this.uniforms.set('vx', {name: 'vx', type: UniformType.FLOAT, value: [vx]})
            this.uniforms.set('vy', {name: 'vy', type: UniformType.FLOAT, value: [vy]})
        })
        broker.createListener('hideCar', '', (hide) => {
            if (offscreenContext.hideCar != hide) {
                offscreenContext.hideCar = hide
                // Artificially toggle a refresh
                broker.sendInternalMessage('refreshOffscreenContent', null)
            }
        })
        broker.createListener('updateMaterial', '', async (shaderInfo) => {
            const shader = new Shader(shaderInfo)
            if (!this.shaders.has(shader.hashString)) {
                this.shaders.set(shader.hashString, new Material(webglContext, shader))
            }
            ...
            shader.info.uniforms.forEach(uniform => {
                // Hack :'(
                if (uniform.name === 'flashStart') {
                    this.uniforms.set(uniform.name, {...uniform, value: [uniform.value[0] - (this.startTime / 1000.0)]})
                } else {
        ...
        this.broker.sendExternalMessage('requestMaterial', null)
    }
