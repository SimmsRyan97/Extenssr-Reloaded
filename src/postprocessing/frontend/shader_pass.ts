...
import { ScopeIndex, Scope } from '@shaderfrog/glsl-parser/dist/parser/parser'
import { Uniform, UniformType } from './uniform'
import hash from 'string-hash'
import { CachedValueStoreClient } from 'storage/value_store'
import defaultFragment from './shaders/simple_fragment.glsl'
import defaultVertex from './shaders/simple_vertex.glsl'
...
export class ShaderError {
    ...
    constructor(message: string, location: ErrorLocation | null = null) {
...
export class StorageToggleableShaderPass implements IToggleableShaderPass {
    ...
    setOverrideStorage(override: boolean): void {
        this.overrideStorage = override
        if (!this.overrideStorage) {
            this.enabled = this.storageEnabled
        } else {
            this.storageEnabled = this.enabled
        }
    }
    setEnabled(enabled: boolean): void {
        this.enabled = enabled
    ...
    storageSetEnabled(enabled: boolean): void {
        this.storageEnabled = enabled
        if (!this.overrideStorage) {
            this.setEnabled(enabled)
        }
    }
    static fromKey<TKeyVal, TKey extends keyof TKeyVal>(storage: CachedValueStoreClient<TKeyVal>, rawString: string, key: TKey, updateFunc: PassUpdateFunc): StorageToggleableShaderPass {
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        const enabled = storage.getCachedValue(key) as any as boolean
        // Just assume it's fine.
        const pass = ShaderPass.fromString(rawString) as ShaderPass
        const forceDisable = async () => {await storage.setValue(key, false as unknown as TKeyVal[TKey])}
        const updatableUniform = new StorageToggleableShaderPass(pass, enabled, updateFunc, forceDisable)
        storage.createListener(key, (newVal) => {
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            const val = newVal as any as boolean
            updatableUniform.storageSetEnabled(val)
        })
